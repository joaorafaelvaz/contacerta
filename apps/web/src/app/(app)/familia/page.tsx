  ...
  return (
    <>
          ...
          {members.map((member) => (
            <li key={member.id} className="flex items-center justify-between py-2">
              <span className="text-sm font-medium text-slate-800 dark:text-slate-200">
                {member.name}
                {member.id === user.id && <span className="ml-1 text-xs text-slate-500 dark:text-slate-400">(você)</span>}
              </span>
      ...
      <Card title="WhatsApp" className="mb-4">
        ...
        {erro === "telefone" && (
          <p className="mt-2 text-sm text-red-600 dark:text-red-400">Número inválido — use DDD + número.</p>
        )}
      </Card>
      ...
      <Card title="Convidar membro">
        ...
        {inviteLink && (
          <div className="mt-3 rounded-md bg-emerald-50 dark:bg-emerald-950/50 p-3">
            <p className="mb-1 text-xs font-medium text-emerald-800 dark:text-emerald-300">Convite gerado — copie e envie:</p>
